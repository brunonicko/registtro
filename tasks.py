...
@task
def conform(c):
    c.run("isort registtro tests ./docs/source/conf.py setup.py -m 3 -l 88 --up --tc --lbt 0")
...
@task
def lint(c):
    c.run("isort registtro tests ./docs/source/conf.py setup.py -m 3 -l 88 --up --tc --lbt 0 --check-only")
    c.run("black registtro --line-length=120 --check")
    ...
    c.run("flake8 registtro --count --ignore=F811,F405,F403,F401,E203,E731,C901,W503 --max-line-length=120 --statistics")
...
def docs(c):
    ...
    os.mkdir(api_docs)
    c.run("sphinx-apidoc registtro --separate --module-first --no-toc --force --output-dir {}".format(api_docs))
